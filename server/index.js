...
const limiter = rateLimit({
  ...
  max: 1000, // Increased limit for open source usage
...
app.get("/api/engines", (req, res) => {
  res.json({
    ...
    capabilities: {
      potrace: {
        description:
          "High-quality bitmap to vector tracing using Potrace algorithm",
        options: [
          "threshold",
          "turdSize",
          "alphaMax",
      ...
      imagetracer: {
        description:
          "Color bitmap to vector tracing with multiple color support",
...
app.post("/api/convert/single", upload.single("file"), async (req, res) => {
  try {
    ...
    switch (mode) {
      case "wrapper":
        svgContent = await wrapImageInSvg(
          req.file.buffer,
    ...
    res.json({
      ...
      outputSize,
      compressionRatio: (
        ((req.file.size - outputSize) / req.file.size) *
        100
      ).toFixed(2),
      processingTime,
      svgContent,
    ...
    for (const file of req.files) {
      try {
        ...
        results.push({
          ...
          outputSize,
          compressionRatio: (
            ((file.size - outputSize) / file.size) *
            100
          ).toFixed(2),
          svgContent,
        });
