...
describe("BatchProcessor", () => {
  ...
  describe("File Management", () => {
    ...
    it("should reject more than 20 files", () => {
      ...
      const tooManyFiles = Array.from(
        { length: 21 },
        (_, i) => new File(["data"], `test${i}.png`, { type: "image/png" })
      );
      ...
      expect(onError).toHaveBeenCalledWith(
        "Maximum 20 files allowed for batch processing"
      );
    ...
    it("should remove individual jobs when not processing", () => {
      ...
      expect(
        result.current.jobs.find((job) => job.id === jobId)
      ).toBeUndefined();
    });
    ...
    it("should not remove jobs while processing", () => {
      ...
      expect(onError).toHaveBeenCalledWith(
        "Cannot remove jobs while processing"
      );
      expect(result.current.jobs).toHaveLength(3);
    });
  ...
  describe("Batch Processing", () => {
    ...
    it("should handle processing errors gracefully", async () => {
      ...
      const invalidFile = new File(["invalid data"], "test.png", {
        type: "image/png",
      });
    ...
    it("should continue processing remaining files after individual failures", async () => {
      ...
      expect(
        result.current.completedJobs.length + result.current.failedJobs.length
      ).toBeGreaterThan(0);
    });
